...
impl ShieldConfig {
    ...
    pub fn load(path: &Path) -> anyhow::Result<Self> {
        ...
        if let Ok(v) = std::env::var("ARTISHIELD_API_TOKEN") {
            cfg.api_token = Some(v);
        }
        if let Ok(v) = std::env::var("ARTISHIELD_ABUSEIPDB_KEY") {
            cfg.mitigations.abuseipdb_key = Some(v);
        }
        if let Ok(v) = std::env::var("ARTISHIELD_HMAC_KEY") {
            cfg.feed.hmac_key = Some(v);
        }
        Ok(cfg)
    }
    ...
    pub fn validate(&self) -> Vec<String> {
        ...
        if self.mitigations.canary_circuits {
            ...
            } else if ep.contains("localhost") || ep.contains("127.0.0.1") || ep.contains("::1") {
                w.push(format!(
...
fn default_db() -> PathBuf {
    PathBuf::from("artishield.db")
}
fn default_log() -> String {
    "artishield=info,warn".into()
}
