...
use artishield::{
    detectors::hs_enumeration::{HsDirSnapshot, HsEnumDetector},
    event::ThreatKind,
    ShieldConfig,
};
...
fn concentrated_hsdir_fires_event() {
    ...
    assert!(
        !evts.is_empty(),
        "10% HSDir concentration should trigger alert"
    );
...
fn dispersed_hsdir_no_alert() {
    ...
    assert!(
        d.analyse_hsdir(&s).is_empty(),
        "Dispersed HSDir nodes should not alert"
    );
}
