...
fn ipv6_no_panic() {
    let hops = vec![
        CircuitHop { fingerprint: "G".into(), addrs: vec!["[2001:db8::1]:443".parse().unwrap()], is_guard: true,  is_exit: false },
        CircuitHop { fingerprint: "M".into(), addrs: vec!["[2001:db8::2]:443".parse().unwrap()], is_guard: false, is_exit: false },
        CircuitHop { fingerprint: "E".into(), addrs: vec!["[2001:db8::3]:443".parse().unwrap()], is_guard: false, is_exit: true  },
    ];
...
fn single_hop_ignored() {
    assert!(det().check_hops(&[hop("1.2.3.4", "G", true, false)]).is_none());
}
