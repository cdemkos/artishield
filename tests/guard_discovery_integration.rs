...
use artishield::{event::ThreatKind, ShieldConfig};
...
fn full_churn_fires_event() {
    let mut d = det();
    let prev_raw: Vec<(String, Option<&str>)> = (0..10)
        .map(|i| (format!("P{i:02}"), Some("1.1.1.1")))
        .collect();
    let curr_raw: Vec<(String, Option<&str>)> = (10..20)
...
fn new_guard_subnet_cluster_fires() {
    ...
    assert!(evts
        .iter()
        .any(|e| matches!(&e.kind, ThreatKind::GuardDiscovery { .. })));
}
